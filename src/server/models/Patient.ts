...
// Pre-save middleware to validate date of birth
...
patientSchema.statics.search = function(query: string) {
  return this.find({
    $and: [
      { isActive: true },
      {
...
patientSchema.statics.findByAgeRange = function(minAge: number, maxAge: number) {
  const today = new Date();
...
patientSchema.methods.deactivate = function(reason?: string, updatedBy?: mongoose.Types.ObjectId) {
  this.isActive = false;
...
// Instance method to reactivate patient
patientSchema.methods.reactivate = function(updatedBy?: mongoose.Types.ObjectId) {
  this.isActive = true;
  if (updatedBy) this.updatedBy = updatedBy;
  this.updatedAt = new Date();
  return this.save();
